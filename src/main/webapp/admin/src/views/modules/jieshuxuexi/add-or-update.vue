<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="结束编号" prop="jieshubianhao">
            <el-input v-model="ruleForm.jieshubianhao" 
                placeholder="结束编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jieshubianhao" label="结束编号" prop="jieshubianhao">
              <el-input v-model="ruleForm.jieshubianhao" 
                placeholder="结束编号" readonly></el-input>
          </el-form-item>
        </div>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.beizhu = this.ruleForm.beizhu.replace(reg,'../../../ssmdls35/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jieshuxuexi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jieshuxuexiCrossAddOrUpdateFlag = false;
						 this.parent.search();
