<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="预定方式" prop="yudingfangshi">
          <el-select :disabled="ro.yudingfangshi" v-model="ruleForm.yudingfangshi" placeholder="请选择预定方式">
            <el-option
                v-for="(item,index) in yudingfangshiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="预定方式" prop="yudingfangshi">
	      <el-input v-model="ruleForm.yudingfangshi"
                placeholder="预定方式" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.yuyuebeizhu = this.ruleForm.yuyuebeizhu.replace(reg,'../../../ssmdls35/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zixishiyuyue/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zixishiyuyueCrossAddOrUpdateFlag = false;
						 this.parent.search();
